...
export const addToWishlist = async (customerprofilecode: number, inventory_id: number) => {
  ...
  const already = _wishlist.some(w => w.InventoryID === inventory_id);
...
export const removeFromWishlist = async (_customerprofilecode: number, wishlistCode: number) => {
  _wishlist = _wishlist.filter(w => w.WishlistCode !== wishlistCode);
